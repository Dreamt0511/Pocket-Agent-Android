...
object TermuxLauncher {
    ...
    fun launchFastAPI(context: Context, mirrorUrl: String = ""): Boolean {
        ...
        val script = buildString {
            ...
            append("  # 杀旧进程再启动，避免重复\n")
            append("  SELF_PID=\$\$; fuser -k 8000/tcp 2>/dev/null; for pid in \$(pgrep -f \"uvicorn\" 2>/dev/null); do [ \"\$pid\" != \"\$SELF_PID\" ] && kill \"\$pid\" 2>/dev/null; done; true\n")
    ...
    fun stopFastAPI(context: Context): Boolean {
        ...
        val script = buildString {
            ...
            append("  # 方法1: fuser 杀端口\n")
            append("  if command -v fuser >/dev/null 2>&1; then\n")
            append("    fuser -k 8000/tcp 2>&1 && echo \"[ok] fuser killed port 8000\"\n")
            append("  fi\n")
            append("  # 方法2: pgrep 匹配进程名（排除自身脚本）\n")
            append("  SELF_PID=$$\n")
            append("  for pid in \$(pgrep -f \"uvicorn\" 2>/dev/null); do\n")
            append("    [ \"\$pid\" != \"\$SELF_PID\" ] && kill \"\$pid\" 2>/dev/null && echo \"[ok] kill PID \$pid\"\n")
            append("  done\n")
            append("  # 验尸: 确认 uvicorn 已死\n")
            append("  sleep 1\n")
            append("  if pgrep -f \"uvicorn\" 2>/dev/null | grep -qv \"\$\$\"; then\n")
