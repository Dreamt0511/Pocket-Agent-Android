...
**Android 仓库（本仓库）推送前必须得到用户明确同意！！！**

- 本地 commit 可以自行完成
- **推送远程必须等用户说"推""push""推送"等确认词，且必须先问"准备推送 xxx，确认吗？"，用户说"确认"才执行**
- **一次同意并不代表以后都同意，每次推送都必须重新确认**
- 用户没说之前，commit 后告知用户已就绪等待其指令即可

**Why:** 本地没有 Android SDK，无法编译验证。之前私自推送导致 CI 编译报错。
...
除此之外的所有目录（包括 Termux 目录）绝对不要碰。

### 3. 不要修改 bootstrap 逻辑
bootstrap() 的原始逻辑是正确的，不要改动。两次修改都导致服务连接失败。

### 4. 排查问题时两库一起查
排查连接/功能问题时，必须同时考虑 Android 和 Python 后端两个仓库。
...
环境要求：JDK 17、Android SDK API 34、Build Tools 32.0.0。项目自带 Gradle Wrapper（8.5）。
...
- `TermuxLauncher` — 通过 Intent 向 Termux 发送 bash 脚本
- `TermuxServiceClient` — OkHttp 客户端，与 localhost:8000 的 FastAPI 通信
- `AgentDaemon` — 协调启动流程和任务执行，管理 DaemonStatus 状态机
- `AppBootstrapper` — 全局单例启动器
...
- `SettingsRepository` — DataStore Preferences 存储 LLM 配置和 MCP 服务地址
...
- Python 运行时内置到 APK 的 assets 目录

## 关键配置

- `local.properties` — SDK 路径（不提交 Git）
- `gradle.properties` — JVM 内存 `-Xmx2048m`
- 签名配置在 `app/build.gradle.kts` 的 `signingConfigs`
- ABI 过滤：仅 `arm64-v8a`
